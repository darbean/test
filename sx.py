...
g_u='18968049597@GDPF.XY' #闪讯帐号
#g_p='' #闪讯密码
g_p='677798'  #闪讯密码(修改此处)
...
def HR30send2():
    ...
    if not ip:
        ...
        ip=ip['ip']
    if not os.path.isfile("C:\Python27\sxkey"): #读取key 没有则使用默认
        key='123456'
    else:
        with open("C:\Python27\sxkey",'r+') as file:
            key=file.read()
    sock=socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    ...
    sock.close()
    with open('C:\Python27\sxkey.txt','w+') as f:
        keypos=mykey.rfind('=')
        if keypos!=-1:
...
def sendHeart():
    ...
    HR30send2()
    time.sleep(1)
    with open('C:\Python27\sxlog.txt','w+') as file: #记录数据
        file.write(HR30send2())
def main(): #包括 显示ip 纯拨号 心跳 拨号并心跳
    arg=sys.argv
    with open('C:\Python27\sxlast.txt','w+') as file:
